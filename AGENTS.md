...
- Add focused tests where practical; for destructive flows, guard with cli flags or dry-run switches. Keep new flags/plumbing consistent across Cobra, execution, and docs.
...
- For passing options use command line options over environment variables. For each option, if possible, there should be a meaningful default to keep the commands short.
