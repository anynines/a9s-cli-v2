...
func deleteCluster(ctx context.Context, cfg Config, opts DeleteOptions) {
	...
	if err := DeleteUserPoolsByTags(ctx, opts.DryRun, cfg.Region, "Name", "userpool-"+cfg.ClusterName); err != nil {
		awsLogger.Fatalf(err, "Failed to clean up Cognito User Pools")
	}
}
	...
	if err == nil && strings.TrimSpace(out) != "" {
		...
		if status == "ACTIVE" {
			if opts.DryRun {
				...
				if errOut, err := runCmd(ctx, true, false, "aws", "eks", "update-kubeconfig",
					"--name", cfg.ClusterName,
...
func kubernetesCleanup(ctx context.Context, cfg Config, opts DeleteOptions) {
	...
	if opts.DryRun {
		...
		if errOut, err := runCmd(ctx, true, false, "helm", "uninstall", cfg.AlbServiceAccountName, "-n", "kube-system"); err != nil {
...
func iamCleanup(ctx context.Context, cfg Config, opts DeleteOptions, accountID string, clusterReachable bool) {
	...
	if opts.DryRun {
		...
		if errOut, err := runCmd(ctx, true, false, "eksctl", "delete", "iamserviceaccount",
	...
	policyArn, err := runCmd(ctx, false, false, "aws", "iam", "list-policies", "--scope", "Local",
	...
	versions, _ := runCmd(ctx, false, false, "aws", "iam", "list-policy-versions",
	...
	for _, v := range strings.Fields(versions) {
		...
		if errOut, err := runCmd(ctx, true, false, "aws", "iam", "delete-policy-version",
	...
	if opts.DryRun {
		...
		if errOut, err := runCmd(ctx, true, false, "aws", "iam", "delete-policy", "--policy-arn", policyArn); err != nil {
...
func deleteNodegroupsAndCluster(ctx context.Context, cfg Config, opts DeleteOptions) {
	...
	ngs, _ := runCmd(ctx, false, false, "aws", "eks", "list-nodegroups",
	...
	for _, ng := range strings.Fields(ngs) {
		...
		if errOut, err := runCmd(ctx, true, false, "aws", "eks", "delete-nodegroup",
			"--cluster-name", cfg.ClusterName,
			"--nodegroup-name", ng,
			"--region", cfg.Region); err != nil {
			awsLogger.Warningf("Failed to request deletion for nodegroup %s: %v\nstderr: %s", ng, err, errOut)
		}
		if errOut, err := runCmd(ctx, false, false, "aws", "eks", "wait", "nodegroup-deleted",
			"--cluster-name", cfg.ClusterName,
			"--nodegroup-name", ng,
			"--region", cfg.Region); err != nil {
	...
	if opts.DryRun {
		...
		if errOut, err := runCmd(ctx, true, false, "aws", "eks", "delete-cluster",
			"--name", cfg.ClusterName,
			"--region", cfg.Region); err != nil {
			awsLogger.Warningf("Failed to request cluster deletion: %v\nstderr: %s", err, errOut)
		}
		if errOut, err := runCmd(ctx, false, false, "aws", "eks", "wait", "cluster-deleted",
			"--name", cfg.ClusterName,
			"--region", cfg.Region); err != nil {
...
func klutchKmsKeyCleanup(cfg Config, ctx context.Context, opts DeleteOptions) {
	...
	out, err := runCmd(ctx, false, false, "aws", args...)
	if err != nil {
...
func scheduleDeletionForKmsKey(ctx context.Context, arn string, opts DeleteOptions, keysDeletionSchedulingFailed []string) []string {
	...
	if errOut, err := runCmd(ctx, false, false, "aws", schedArgs...); err != nil {
...
func retagKmsKey(cfg Config, ctx context.Context, arn string, opts DeleteOptions, keysRetaggingFailed []string) []string {
	...
	if errOut, err := runCmd(ctx, false, false, "aws", retagArgs...); err != nil {
...
func disableKmsKey(ctx context.Context, arn string, opts DeleteOptions, keysDisablingFailed []string) ([]string, bool) {
	disableArgs := appendRegion([]string{"kms", "disable-key",
		"--key-id", arn},
		opts.Region)
	if errOut, err := runCmd(ctx, false, false, "aws", disableArgs...); err != nil {
		// If the key is already pending deletion, we can ignore this error
		if strings.Contains(errOut, "KMSInvalidStateException") && strings.Contains(errOut, "is pending deletion.") {
...
func findKlutchVPC(cfg Config, ctx context.Context, region string) string {
	...
	if err != nil || out == "" || out == "None" {
		awsLogger.Infof("No VPC with '%s=%s' and 'Name=%s' found (stderr: %s)", klutchTagKey, klutchTagValue, resourceName(cfg, "vpc"), out)
		return ""
	}
...
func deleteVPCEndpoints(ctx context.Context, vpcID string, opts DeleteOptions) {
	...
	args := []string{"ec2", "describe-vpc-endpoints",
		...
		"--output", "text"}
	args = appendRegion(args, opts.Region)
	eps, _ := runCmd(ctx, false, false, "aws", args...)
	for _, ep := range strings.Fields(eps) {
		...
		args = appendRegion(args, opts.Region)
		errOut, err := runCmd(ctx, true, false, "aws", args...)
		if err != nil {
...
func deleteLoadBalancers(ctx context.Context, vpcID string, opts DeleteOptions) []string {
	...
	args := []string{"elbv2", "describe-load-balancers",
		...
		"--output", "text"}
	args = appendRegion(args, opts.Region)
	lbs, _ := runCmd(ctx, false, false, "aws", args...)
	for _, lb := range strings.Fields(lbs) {
		...
		args = []string{"elbv2", "describe-load-balancers",
			...
			"--output", "text"}
		args = appendRegion(args, opts.Region)
		lbDNS, _ := runCmd(ctx, false, false, "aws", args...)
		if lbDNS != "" && lbDNS != "None" {
		...
		args = []string{"elbv2", "describe-target-groups",
			...
			"--output", "text"}
		args = appendRegion(args, opts.Region)
		tgs, _ := runCmd(ctx, false, false, "aws", args...)
		...
		if opts.DryRun {
			...
			args = appendRegion(args, opts.Region)
			if errOut, err := runCmd(ctx, true, false, "aws", args...); err != nil {
...
func waitForELBENIs(ctx context.Context, vpcID string, opts DeleteOptions) {
	...
	for range 10 {
		args := []string{"ec2", "describe-network-interfaces",
			...
			"--output", "text"}
		args = appendRegion(args, opts.Region)
		enis, _ := runCmd(ctx, false, false, "aws", args...)
...
func deleteNATGateways(ctx context.Context, vpcID string, opts DeleteOptions) []string {
	...
	args := []string{"ec2", "describe-nat-gateways",
		...
		"--output", "text"}
	args = appendRegion(args, opts.Region)
	natEIPs, _ := runCmd(ctx, false, false, "aws", args...)
	...
	args = []string{"ec2", "describe-nat-gateways",
		...
		"--output", "text"}
	args = appendRegion(args, opts.Region)
	ngws, _ := runCmd(ctx, false, false, "aws", args...)
	...
	for _, ng := range strings.Fields(ngws) {
		...
		for {
			args = []string{"ec2", "describe-nat-gateways",
				...
				"--output", "text"}
			args = appendRegion(args, opts.Region)
			state, _ := runCmd(ctx, false, false, "aws", args...)
...
func deleteInternetGateway(ctx context.Context, vpcID string, opts DeleteOptions) {
	...
	args := []string{"ec2", "describe-internet-gateways",
		...
		"--output", "text"}
	args = appendRegion(args, opts.Region)
	igwID, _ := runCmd(ctx, false, false, "aws", args...)
	if igwID == "" || igwID == "None" {
		return
	}
	if opts.DryRun {
...
func deleteRouteTables(ctx context.Context, vpcID string, opts DeleteOptions) {
	...
	args := []string{"ec2", "describe-route-tables",
		...
		"--output", "text"}
	args = appendRegion(args, opts.Region)
	nonMainRTs, _ := runCmd(ctx, false, false, "aws", args...)
	...
	for _, rt := range strings.Fields(nonMainRTs) {
		...
		args = []string{"ec2", "describe-route-tables",
			...
			"--output", "text"}
		args = appendRegion(args, opts.Region)
		assocs, _ := runCmd(ctx, false, false, "aws", args...)
...
func deleteTenantOperatorRole(ctx context.Context, cfg Config, accountID string, opts DeleteOptions) {
	...
	_, _ = runCmd(ctx, true, false, "aws", "iam", "delete-role-policy",
		"--role-name", roleName,
		"--policy-name", "TenantOperatorInline")

	if errOut, err := runCmd(ctx, true, false, "aws", "iam", "delete-role", "--role-name", roleName); err != nil {
...
func deleteOIDCProvider(ctx context.Context, cfg Config, accountID string, opts DeleteOptions, clusterReachable bool) {
	...
	if errOut, err := runCmd(ctx, true, false, "aws", "iam", "delete-open-id-connect-provider",
...
func deleteSubnets(ctx context.Context, vpcID string, opts DeleteOptions) {
	...
	args := []string{"ec2", "describe-subnets",
		...
		"--output", "text"}
	args = appendRegion(args, opts.Region)
	subnets, _ := runCmd(ctx, false, false, "aws", args...)
...
func deleteSecurityGroups(ctx context.Context, vpcID string, opts DeleteOptions) {
	...
	args := []string{"ec2", "describe-security-groups",
		...
		"--output", "text"}
	args = appendRegion(args, opts.Region)
	sgs, _ := runCmd(ctx, false, false, "aws", args...)
		...
		for attempt := 1; attempt <= 2; attempt++ {
			if errOut, err := runCmd(ctx, true, false, "aws", args...); err != nil {
				if attempt == 1 && isAuthError(errOut) {
					...
					if idErrOut, idErr := runCmd(ctx, false, false, "aws", "sts", "get-caller-identity", "--output", "text"); idErr != nil {
...
func findHostedZoneIdByClusterNameTag(ctx context.Context, opts DeleteOptions) string {
	...
	out, err := runCmd(ctx, false, false, "aws", "resourcegroupstaggingapi", "get-resources",
...
func removeClusterNameTagFromHostedZone(ctx context.Context, zoneID string, opts DeleteOptions) {
	...
	if out, err := runCmd(ctx, false, false, "aws", "route53", "change-tags-for-resource",
...
func findHostedZoneIDByName(ctx context.Context, hostedZoneName string) string {
	...
	out, err := runCmd(ctx, false, false, "aws", "route53", "list-hosted-zones-by-name",
		"--dns-name", normalized,
		"--query", query,
		"--output", "text")
	if err != nil {
...
func listHostedZoneRecords(ctx context.Context, zoneID string) ([]map[string]interface{}, error) {
	out, err := runCmd(ctx, false, false, "aws", "route53", "list-resource-record-sets",
...
func deleteDNSRecords(ctx context.Context, zoneID, hostedZoneName string, opts DeleteOptions) {
	...
	if errOut, err := runCmd(ctx, true, false, "aws", "route53", "change-resource-record-sets",
		"--hosted-zone-id", zoneID,
...
func deleteHostedZone(ctx context.Context, zoneID, hostedZoneName string, opts DeleteOptions) {
	...
	if errOut, err := runCmd(ctx, true, false, "aws", "route53", "delete-hosted-zone", "--id", zoneID); err != nil {
...
func discoverKlutchCertificateARN(ctx context.Context, opts DeleteOptions) string {
	...
	for arn := range strings.FieldsSeq(out) {
		...
		tagsOut, tagErr := runCmd(ctx, false, false, "aws", tagArgs...)
		if tagErr != nil {
...
func runDiagnostics(ctx context.Context, vpcID, region string) {
	...
	for _, d := range diagnostics {
		...
		if errOut, err := runCmd(ctx, false, false, "aws", args...); err != nil {
...
func DeleteUserPoolsByTags(ctx context.Context, dryRun bool, region, key string, values ...string) error {
	...
	if err != nil {
		...
		return fmt.Errorf("could not discover user pools by tag: %w", err)
	}
	...
	for _, arn := range arns {
		...
		if err != nil {
			...
			deletionFailures = append(deletionFailures, fmt.Errorf("could not delete user pool %s: %w", arn, err))
		}
	}

	if len(deletionFailures) > 0 {
		return errors.Join(deletionFailures...)
	}
...
func deleteUserPoolByArn(ctx context.Context, arn string, region string, dryRun bool) error {
	...
	if err != nil {
		...
		return fmt.Errorf("could not describe user pool %s for retrieving its domain: %w: %s", poolID, err, domainOutput)
	}
	...
	if domain != "" && strings.ToLower(domain) != "none" && strings.ToLower(domain) != "null" {
		...
		if output, err := runCmd(ctx, true, false, "aws",
			"cognito-idp", "delete-user-pool-domain",
			"--region", region,
			"--user-pool-id", poolID,
			"--domain", domain); err != nil {
			return fmt.Errorf("could not delete domain %q for pool %s: %w: %s", domain, poolID, err, output)
		}
	}
	...
	if output, err := runCmd(ctx, true, false, "aws",
		...
		return fmt.Errorf("could not delete Cognito user pool %s: %w: %s", poolID, err, output)
	}
