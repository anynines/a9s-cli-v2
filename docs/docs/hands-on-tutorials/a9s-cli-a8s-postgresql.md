...
In this tutorial you will be using the `a9s` CLI to fascilitate the creation of both a local Kubernetes cluster and a PostgreSQL database instance. 
...
In this section you will create a Kubernetes cluster with a8s PostgreSQL and alls its dependencies:
...
**Note**: The `a9s CLI` does not shield you the YAML specs is generated. Quite the opposite, it is intended to provide you with meaningful templates to start with. **You can find all YAML specs generated by the `a9s CLI` in the `usermanifests` folder in your a9s workding directory**:
...
The `postgresql` object named `clustered-instance`, as the name suggests, represents your PostgreSQL instance. It is implemented by a set of Kuberbenetes Services and a StatefulSet.
...
Especially, look athe the `Labels` section in the output:
...
Note that the values in the `data` hash aren't readable right away as the are base64 encoded. Values can be decoded using the `base64` command, for example:
...
The demo app conists of a Kubernetes Service and a Deployment both named `demo-app`.
...
Although not the preferred way to load seed data into a production database, during development it might be handy to execute a SQL file to a PostgreSQL instance. This allows executing one or mulitple SQL statements convienently. 
...
Again, the `pg apply` commands are not meant to interact with production databases but may become handy than during debugging and local development.
...
Again, apply the `COUNT` or reaload the website to see that the restore has brought back all blog posts.
...
With just a few commands, you have created a local Kubernetes cluster, installed the a8s PostgreSQL Control Plane including all it's dependencies. Furthermore, you have provisioned an PostgreSQL cluster consisting of three Pods providing you with an asynchronous streaming cluster supporting automatic failure detection, lead-election and failover. Deploying the demo application you've also experienced the convience of Service Bindings and their automatic creation of Kubernetes Secrets. The backup and restore experiment then illustrated how effortless handling a production database database can be.
