...
The working directory is stored in the `~/.a9s` configuration file. The working directory contains all resources downloaded and generated by the `a9s` CLI.
...
Will generate a YAML spec called `usermanifests/a8s-pg-instance-sample-pg-cluster.yaml` with the following content:
